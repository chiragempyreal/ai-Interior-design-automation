...
const Projects: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get(`/admin/projects?page=${page}&limit=${limit}`);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-3">
          <button className="flex items-center px-4 py-2 bg-white border border-border rounded-lg text-sm font-medium text-text hover:bg-background-light transition-colors">
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-border">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-8 text-center text-text-secondary">No projects found.</td>
                </tr>
              ) : (
                data?.projects.map((project: Project) => (
                  <tr key={project._id} className="hover:bg-background-light/50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <button className="text-text-secondary hover:text-primary transition-colors p-1">
                        <Eye className="w-4 h-4" />
                      </button>
                      <button className="text-text-secondary hover:text-primary transition-colors p-1 ml-2">
